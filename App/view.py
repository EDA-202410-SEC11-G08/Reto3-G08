...
if __name__ == "__main__":
    ...
    while working:
        ...
        if int(inputs) == 1: # CARGA DE DATOS ------------------------------------------------------ CAMBIAR TAMANO DE DATOS CARGADOS
            ...
            if mem:
                control, deltaTime, deltaMemory=controller.req_3(control,codPais,experticia,int(num), mem)
            else:
                control, deltaTime= controller.req_3(control,codPais,experticia,int(num), mem)
            size=control['model']['REQ3'][0]
            print("\nTotal de ofertas pulicadas: " + str(size))
            print("Tiempo [ms]: ", f"{deltaTime:.3f}")
            if (mem == True): print("Memoria [kB]: ", f"{deltaMemory:.3f}")  
            table=print_req_3(control['model']['REQ3'][1],size, mem)
            print(tabulate(table))

        elif int(inputs) == 5:
            print("\nBuscando ofertas laborales ... ")
            num=int(input("Ingrese el número de ofertas que quiera consultar: "))
            print("\nBuscando ofertas más recientes para un país teniendo en cuenta un país en específico: ")
            ciudad = input("Ingrese la ciudad: ")
            tipo = input("Ingrese el tipo de ubicación de trabajo (remote, partialy, remote, office): ")
            if mem:
                lst, size, deltaTime, deltaMemory=controller.req_4(control, ciudad, tipo, num, mem)
            else:
                lst, size, deltaTime= controller.req_4(control, ciudad, tipo, num, mem)
            
            print("\nTotal de ofertas pulicadas: " + str(size))
            print("Tiempo [ms]: ", f"{deltaTime:.3f}")
            if (mem == True): print("Memoria [kB]: ", f"{deltaMemory:.3f}")  
            table=print_req_4(lst,size, mem)
            print(tabulate(table))
