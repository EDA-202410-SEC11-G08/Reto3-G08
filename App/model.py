...
def req_3(catalog, country_code, experience_level, num):
    ...
    country_jobs = om.values(catalog["dateIndex"])
    ...
    return total_jobs, selected_jobs
...
def req_4(catalog, city_name, location_type, num):
    ...
    for date_entry in lt.iterator(catalog["dateIndex"]):
        for job in lt.iterator(date_entry['lstjobs']):
            if (job['ciudad_empresa'] == city_name and job['tipo_ubicacion'] == location_type):
                lt.addLast(filtered_jobs, job)
